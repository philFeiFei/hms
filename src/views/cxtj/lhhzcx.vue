<template>
  <div class="app-container">
    ...
    <el-table size="mini" v-loading="listLoading" :key="tableKey" :data="currentPageList" border fit highlight-current-row style="width: 100%;" :height="tableHeight">
      ...
      <el-table-column :label="$t('table.lhsj')" min-width="90px" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.lhsj | parseTime('{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.lhr')" min-width="70px">
        <template slot-scope="scope">
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['订单日期', '订单编号', '色号', '网底款式', '网底尺寸', '发长', '领活时间', '领活人', '数量']
        const filterVal = ['ddrq', 'ddbh', 'sh', 'wdks', 'wdcc', 'fc', 'lhsj', 'lhr', 'sl']
